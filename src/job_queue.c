...
int job_queue_init(struct job_queue *job_queue, int capacity) {
    ...
    if (job_queue->buffer == NULL) { return ENOMEM; }
    ...
    if (pthread_mutex_init(&job_queue->lock, NULL) != 0) { free(job_queue->buffer); return -1; }
    if (pthread_cond_init(&job_queue->not_empty, NULL) != 0) { pthread_mutex_destroy(&job_queue->lock); free(job_queue->buffer); return -1; }
